...
exports.createAnOrder = async(req, res, next) => {
    try {
        ...
        const {shippingInfo, paymentInfo, paidAt} = req.body;
        const carts = await Cart.find({userId:req.user.id});
        //loop through the carts and create order for each item while updating the stock of the products

        let order = {};
        carts.map(async (item, ind)=>{
            const product = await Product.findById(item.productId);
            order = await Order.create({shippingInfo, paymentInfo, paidAt, itemsPrice: item.price, productId:item.productId, companyId:product.creatorId, quantity:item.quantity, status:"Processing",
                userId:req.user.id  
            });  
