...
exports.createProduct = async (req, res, next) => {
    try {
        ...
        const {name, description, price, category, subcategory, stock, height, width, length} = req.body;

        const product = await Product.create({
            name, description, price, category, subcategory, stock, height, width, length, creatorId:req.user.id,
            images:imagesLink 
        });
...
exports.updateProduct = async (req, res, next) => {
    try {
        ...
        for (let i = 0; i< oldImagesFromDB.length; i++){
         ...
         const {name, description, price, subcategory, stock, height, width, length} = req.body;
     ...
     const productUpdated = await Product.findByIdAndUpdate(req.params.id, { name, description, price, 
         subcategory, stock, height, width, length,
