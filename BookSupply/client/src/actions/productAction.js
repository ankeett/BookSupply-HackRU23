...
//CREATE A PRODUCT
export const createProduct = (name, description, price, category, subcategory, stock, height, width, length, images) => async(dispatch) => {
    try {
        dispatch({type:CREATE_PRODUCT_REQUEST});
        const config = {headers:{"Content-type":"application/json"},  withCredentials: true};
        const {data} = await axios.post(
            `${host}/api/v1/product/create`,
            {name, description, price, category, subcategory, stock, height, width, length, images},
            config
        )  
...
//UPDATE PRODUCT
export const updateProduct = (id, name, description, price, subcategory, stock, height, width, length, images, newImages) => async(dispatch) => {
    try {
        dispatch({
        ...
        const {data} = await axios.put(
                ...
                {name, description, price, subcategory, stock, height, width, length, images, newImages},
                config
            );
