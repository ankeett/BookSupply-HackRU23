...
import {changePasswordCompany, clearErrors} from '../../actions/companyAction'
import MetaData from '../../MetaData';

const Reset = () => {
    const dispatch = useDispatch();
    const {error, isUpdated, isLoading} = useSelector(state=>state.forgotPw);
    ...
    const changePasswordSubmit = (e) => {   
        ...
        if (newPassword === confirmPassword){
            dispatch( changePasswordCompany(prevPassword, newPassword));
        }
        else{
    ...
    return (
        <>
        <MetaData title = "Change Password - Company"/>
