...
const Update = () => { 
  ...
  const handleSubmit = (e) =>{
    ...
    dispatch(updateProduct(productId, name, description, price,subcategory,stock,height,width,length, image, newImages));
  }
  ...
  return (
    ...
    { product && 
        ...
        <div className='p-[20px] mb-[20px] text-center'>
          <Typography className='text-center'>Update a Listing</Typography>
        </div>
        <form action="" ref={formRef}>
            <Grid container spacing ={2} className='gap-[30px]'>
                ...
                <TextField required type="number" fullWidth name='price' label="Price" value={price} onChange = {(e)=>{setPrice(e.target.value)}}
                  InputProps={{
                    startAdornment:(
            ...
            <div className='flex flex-col mt-16 gap-8 align-middle justify-center'>
              <Button fullWidth onClick={handleSubmit} className='text-white bg-blue-700 rounded-lg normal-case'>Submit</Button>
            </div>
        </form>
      </Paper>
    }
