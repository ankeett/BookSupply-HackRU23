...
const Orders = () => {
  ...
  return (
    ...
    <tr>
      ...
      {user?.role === "user" &&
            <th className="bg-blue-100 border text-left px-8 py-4">Seller Id</th>}

      <th className="bg-blue-100 border text-left px-8 py-4">Price</th>
      <th className="bg-blue-100 border text-left px-8 py-4">Payment Status</th>
    ...
    { order && order?.map((item,index)=>(
      ...
      <td className="border px-8 py-4">{item.companyId}</td>}
        ...
        <td className="border px-8 py-4">${item.itemsPrice}</td>
        <td className="border px-8 py-4">{item.paymentInfo.status}</td>
  ...
  </table>
  {
    ...
    <Paper className="m-[50px] p-[50px] rounded-[15px] " elevation={6}>
      ...
      <Button className="text-white bg-blue-700 normal-case hover:scale-105 mt-3" onClick={handleSubmit}>Submit</Button>
      </Paper>
      </div>
          : ""
  }
